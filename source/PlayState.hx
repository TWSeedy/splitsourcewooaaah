...
class PlayState extends MusicBeatState
{
	...
	public var youjustgotstunnedbitch:Bool = false;
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'division':
				dialogue = CoolUtil.coolTextFile(Paths.txt('division/splittalkswoah'));
		}
            ...
            case 'unactivebg':
                {
                        defaultCamZoom = 0.9;
                        curStage = 'unactivebg';
                        var splitOne:FlxSprite = new FlxSprite(-650, -350).loadGraphic(Paths.image('Crystal_bg'));
                        splitOne.setGraphicSize(Std.int(splitOne.width * 0.7));
						splitOne.antialiasing = true;
                        splitOne.scrollFactor.set(0.2, 0.2);
                        splitOne.active = false;
                        add(splitOne);

                        var splitFront:FlxSprite = new FlxSprite(-300, -200).loadGraphic(Paths.image('split_floor'));
                        splitFront.setGraphicSize(Std.int(splitFront.width * 0.9));
                        splitFront.updateHitbox();
                        splitFront.antialiasing = true;
                        splitFront.scrollFactor.set(0.9, 0.9);
                        splitFront.active = false;
                        add(splitFront);

						lungcarcinoma = new FlxSprite(21,21);
						lungcarcinoma.frames = Paths.getSparrowAtlas('splitAnims/lung_cancer');
						lungcarcinoma.animation.addByPrefix('endmysufferingaahhh', 'lungcancer', 24, false);
						lungcarcinoma.antialiasing = true;
                }
			case 'sonicbg':
				{
			    		var posX = -600;
	    				...
	    				var bg:FlxSprite = new FlxSprite(posX, posY);
     					bg.frames = Paths.getSparrowAtlas('Sonicd_Up');
			    		bg.animation.addByPrefix('idle', 'Sonicd Up bg', 24);
			...
			case 'activebg':
				{
	    				...
	    				var bg:FlxSprite = new FlxSprite(posX, posY);
     					bg.frames = Paths.getSparrowAtlas('Pieces_bg');
			    		bg.animation.addByPrefix('idle', 'Pieces bg', 24);
		    			bg.animation.play('idle');
			     	   	bg.scrollFactor.set(0.2, 0.2);
				    	bg.scale.set(0.7, 0.7);
		     			add(bg);
		...
		switch (SONG.player2)
		{
			...
			case 'spirit':
				...
				camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
			case "split":
				dad.y += -90; //dont you dare say anything about this
				dad.x += -40;
			case "split-tired":
				dad.y += -80;
				dad.x += -40;
	...
	private function handleInput(evt:KeyboardEvent):Void { // this actually handles press inputs
		...
		if (PlayStateChangeables.botPlay || loadRep || paused || youjustgotstunnedbitch)
			return;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (SONG.song != 'sonicd-up')
			{
				babyArrow.frames = Paths.getSparrowAtlas('colNotes/NOTE_assets');
		...
		private function keyShit():Void // I've invested in emma stocks
			{
				...
				{
					if (boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.animation.curAnim.name.endsWith('miss') && !youjustgotstunnedbitch)
						boyfriend.playAnim('idle');
				}
		 
				playerStrums.forEach(function(spr:FlxSprite)
				{
					if (pressArray[spr.ID] && spr.animation.curAnim.name != 'confirm' && !youjustgotstunnedbitch)
	...
	*/
	...
	function holyshitbullet():Void
		{
			dad.playAnim('shoot', true);
			FlxG.sound.play(Paths.sound('shootin'));
			new FlxTimer().start(0.01, function(tmr:FlxTimer) //doing this so animation **will** play
				{
					boyfriend.playAnim('dodge', true);
				});
		}

		function ahhhbullet():Void
			{
				combo = 0;
				misses++;
				health -= 0.66;
				youjustgotstunnedbitch = true;
				new FlxTimer().start(0.05, function(tmr:FlxTimer) //i am so fucking sorry
					{
						playerStrums.forEach(function(spr:FlxSprite)
							{
								spr.alpha -= 0.1;
						...
						if (health >= 0)
							{
								...
								if (boyfriend.animation.curAnim.name == 'hit' && boyfriend.animation.curAnim.finished && youjustgotstunnedbitch)
									{
										boyfriend.animation.play('hitLoop');
									}
							}
					});
			}
	...
	function lungsdying():Void
	{
		lungcarcinoma.setPosition(dad.x,dad.y);
		remove(dad);
		add(lungcarcinoma);
		lungcarcinoma.animation.play('endmysufferingaahhh');
	}
	...
	override function beatHit()
	{
		...
		if (curStep == 1168 && curSong == 'Division')
			{
				lungsdying();
			}
