...
class OptionsMenu extends MusicBeatState
{
	...
	var splitNBf:FlxSprite;
	
	override function create()
	{
		instance = this;

		bg = new FlxSprite(-100).loadGraphic(Paths.image('menuBGOptions'));
        bg.scrollFactor.x = 0;
        bg.scrollFactor.y = 0;
		...
		bgFront = new FlxSprite(-100).loadGraphic(Paths.image('menuBGFrontOptions'));
		...
		splitNBf = new FlxSprite(0, 0);
		splitNBf.frames = Paths.getSparrowAtlas('splitNBfOptions');
		splitNBf.antialiasing = true;
		splitNBf.animation.addByPrefix('bump', 'splitnbfbumpwoa', 24);
		splitNBf.animation.play('bump');
		splitNBf.setGraphicSize(Std.int(splitNBf.width * 0.75));
		splitNBf.updateHitbox();
		splitNBf.screenCenter();
		add(splitNBf);
